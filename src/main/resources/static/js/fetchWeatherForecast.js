...
function updateWeatherForecast() {
    $.ajax({
        ...
        success: function(result) {
            parseJSON(result);
            $("#weather_report_headline").text(weather.city + "'s Weather Report");
    ...
    setTimeout(updateWeatherForecast, 30000);
};


function parseJSON(weatherJSON){
    ...
    for(var i=0; i < weather.max_temp.length; i++){
        weather.max_temp[i] = Math.round(weather.max_temp[i] - 273.16);
    }
